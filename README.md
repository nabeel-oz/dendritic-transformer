...
![A standard flat-sum neuron beside a dendritic one that sorts inputs into branches, processes each, then combines.](docs/assets/two-neurons.svg)
...
![Standard and dendritic neurons drawn at the same parameter budget; only the internal wiring differs.](docs/assets/parameter-parity.svg)
...
![Phase-1 substitutive design: branches replace the direct path, so the whole signal never reaches the neuron.](docs/assets/phase1-starvation.svg)
...
![Parallel vs sequential wiring of the dendritic add-on; in both, the direct x path is preserved.](docs/assets/parallel-vs-sequential.svg)
...
![Validation loss on character-level text — every variant lands within a whisker of the SwiGLU baseline](docs/assets/phase2_char_loss.png)
...
![The toy harmony language: predict the chord from a nested bracket pair, with one specific pairing held out of training.](docs/assets/harmony-grammar.svg)
...
![Compositional generalisation gap by variant — the augmentative variants all beat the standard neuron, but the plain control edges out the compartmental one](docs/assets/test2_pcfg_gap.png)
...
![Toy-language generalisation gap across the budget-and-compartment sweep — the plain large add-on generalises best; adding compartments and branches makes it worse](docs/assets/sweep_pcfg_gap.png)
...
![Diagram of the ArborFFN dendritic primitive: each branch learns a local cluster of inputs and multiplies two projections of it to detect coincidence; the combined result then feeds the main SwiGLU soma alongside the preserved input.](docs/assets/arbor-ffn.svg)
...
![Timeline of the project's turns, coloured by whether Nabeel, Claude, or a reviewing model made each call.](docs/assets/project-arc.svg)
...
- **Full technical reports**, with every number, error bar, and threat-to-validity: [REPORT_PHASE1.md](REPORT_PHASE1.md), [REPORT_PHASE2.md](REPORT_PHASE2.md), and [REPORT_PHASE3.md](REPORT_PHASE3.md) (the reviewer pass — the depth control and the truer dendrite).
- **Design briefs**, written before any code, with predictions stated up front: [PROJECT_BRIEF_PHASE1.md](PROJECT_BRIEF_PHASE1.md) and [PROJECT_BRIEF_PHASE2.md](PROJECT_BRIEF_PHASE2.md).
- **Build log** — what was done at each step: [PROJECT_LOG.md](PROJECT_LOG.md).
